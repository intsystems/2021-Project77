def DelayEmbeddingPhaseIdentifier(
    ...
    Вход: 
        x - временной ряд
        nlags - длина временного ряда
        epsilon - гиперпараметр соседства точек фазового пространства
        min_dim - оптимальная размерность фазового пространства
        l1, l2, l3 - весовые коэффициенты функций потерь
        make_plots - отрисовка графиков
        return_variance_model - возвращает ожидаемую траекторию
        return_expectation_model - возвращает дисперсию
        
     Выход:
        Словарь с массивом значений извлеченных фаз, ожидаемой траекторией и дисперсией
        
    Основная идея алгоритма заключается в присвоении фазы новым точкам временного ряда 
    в соответствии с их близостью с аппроксимирующей моделью в фазовом пространстве. 
    Предлагается определять значение фазы, исходя из функций потерь L1, L2, L3.
    Функция потерь L1 опирается на предположение монотонном росте фаз.
    Так как сигнал является квазипериодическим, то логично разрешить уменьшение фазы
    в пределах некоторой фиксированной величины.
    Функция потерь L2 опирается на предположение о близости фазы соседних точек.
    Функция потерь L3 учитывает расстояние от текущей точки до точек аппроксимирующей модели.
    """
